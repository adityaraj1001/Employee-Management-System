...
def home():
    ...
    max_salary = db.session.query(
        func.coalesce(func.max(Employee.salary), 0)
    ).scalar()

    min_salary = db.session.query(
        func.coalesce(func.min(Employee.salary), 0)
    ).scalar()
    ...
    if dept_rows:
        largest = max(dept_rows, key=lambda x: x[1])
        largest_department = largest[0]
        largest_department_count = largest[1]

    # ===========================
    # Salary Distribution
    # ===========================

    salary_buckets = [0, 0, 0, 0]
